...
### Оновлено
- **[Розробка власного скіла](content/Skill_Development_Guide.md)** — додано опис чотирьох станів скілів у панелі `/skills` (`on`, `name-only`, `user-only`, `off`) та роль стану `name-only` для економії токенів
- **[Архітектура субагентів](content/Subagents_Architecture.md)** — додано розділ про вкладених субагентів (глибокі дерева субагентів, вимоги до `allowedTools` та правила передачі контексту)
