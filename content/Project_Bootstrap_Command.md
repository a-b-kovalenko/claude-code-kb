...
## Порівняння з вбудованою `/init`

Вбудована команда `/init` створює або оновлює `CLAUDE.md` на основі аналізу існуючого проєкту — одна задача, одна дія. `/init-project` охоплює повний bootstrap:

| | `/init` | `/init-project` |
| :--- | :---: | :---: |
| Створює CLAUDE.md | ✅ | ✅ |
| git init, .gitignore, хуки | ❌ | ✅ |
| .claude/settings.json, .workspace/ | ❌ | ✅ |
| docs/adr/, ADR-0001 | ❌ | ✅ |
| MCP-сервери | ❌ | ✅ |
| Нові проєкти (без коду) | ❌ | ✅ |
| Повний аудит існуючого проєкту | ❌ | ✅ |

Для retrofit: `/init` — хороший старт якщо потрібен лише CLAUDE.md. `/init-project` вирішує весь checklist і пропонує ретроспективні ADR.
...
- **Існуючий** → проведи повний аудит згідно з [Гайд аудиту проєкту](Project_Audit_Guide.md): git-історія, build-файли, структура, CI/CD, конфіги, код. Після аудиту постав лише одне запитання: **чи потрібна документація Antora?**
...
- [📐 Методологія ADR](ADR_Guide.md) — деталі методології ADR
