...
**Tool** — функція для агента, **Resource** — URI-адресовані дані, **Prompt** — готовий workflow для користувача. Детальний фреймворк вибору: [MCP примітиви: Tools / Resources / Prompts](MCP_Primitives.md).
...
> **stdio і безпека:** локальний stdio-транспорт запускає сервер як дочірній процес і спілкується через стандартні потоки. Дані не виходять за межі машини розробника — максимально безпечний варіант для роботи з чутливою інфраструктурою.
...
Discovery відбувається один раз при старті сесії — не при кожному виклику. Якщо ви змінили код сервера або додали новий інструмент під час активної сесії, Claude Code не дізнається про це автоматично — потрібен рестарт.
...
| 7 | MCP Server | Зовнішнє API / Інфраструктура | Реальний запит (HTTP, SQL, тощо) |
| 8 | MCP Server | Our Server | `CallToolResult` — дані відповіді |
| 9 | Our Server | Claude | `toolResult` — результат передається моделі |
| 10 | Claude | User | Фінальна відповідь "Your repositories are..." |

**Ключове:** Claude ніколи не звертається до зовнішніх систем напряму. Він повертає `ToolUse` блок — рішення що викликати. Реальний запит робить MCP-сервер.
