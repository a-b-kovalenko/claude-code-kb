...
Якість відповіді Claude прямо залежить від якості запиту. Шість правил ефективного промптингу для задач розробки: вказуйте файли явно, формулюйте constraints мовою коду, задавайте критерій Done у самому запиті, обмежуйте scope словом "Stop", посилайтесь на конкретні рядки, а не абстрактні описи.
...
Коли scope заздалегідь невідомий — спочатку запустіть [Explorer](Explorer_Subagent_Example.md), щоб отримати точний список файлів, а вже потім давайте завдання на реалізацію.
...
Якщо ви самі не знаєте де проблема — попросіть [`test-runner`](Test_Runner_Subagent_Example.md) надати "Root cause" та "Suggested fix area" і потім давайте завдання на виправлення.
